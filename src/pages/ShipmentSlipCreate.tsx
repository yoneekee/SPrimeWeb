...
import { ArrowLeft, Plus, Trash2, Save, Send, FileText, AlertTriangle } from "lucide-react";
...
const ShipmentSlipCreate = () => {
  ...
  } = useForm<ShipmentSlipFormValues>({
    resolver: zodResolver(shipmentSlipSchema),
    defaultValues: {
  ...
  const addItem = (catalogItem: CatalogItem) => {
    ...
    setDetails((prev) => [
      ...prev,
      { id: nextId, itemCode: catalogItem.code, itemName: catalogItem.name, stockQty: catalogItem.stockQty || 0, shipQty: 1, unitPrice: catalogItem.price, salesAmount: catalogItem.price, inTransit: true, lotNo: "" },
    ]);
    setNextId((n) => n + 1);
  ...
  const updateShipQty = (id: number, qty: number) => {
    setDetails((prev) => prev.map((d) => d.id === id ? { ...d, shipQty: qty, salesAmount: qty * d.unitPrice } : d));
  };
  ...
  const updateTransit = (id: number, checked: boolean) => {
    setDetails((prev) => prev.map((d) => (d.id === id ? { ...d, inTransit: checked } : d)));
  };
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate("/production/shipping")} className="gap-1.5 text-muted-foreground hover:text-foreground">
              <ArrowLeft className="w-4 h-4" /> 一覧
            </Button>
            <Separator orientation="vertical" className="h-6" />
            <div>
              <h1 className="text-lg font-bold text-foreground">新規出庫伝票作成</h1>
              <p className="text-xs text-muted-foreground mt-0.5">完成品出庫のための新規伝票を作成します</p>
            </div>
          </div>
        ...
        <Card className="border-border bg-card">
          ...
          <CardContent className="px-4 pb-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground">出庫希望日 <span className="text-destructive">*</span></label>
                <DatePicker value={shipDate} onChange={(d) => setValue("shipDate", d ? format(d, "yyyy-MM-dd") : "", { shouldValidate: true })} />
                <FormError message={errors.shipDate?.message} />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground">得意先 <span className="text-destructive">*</span></label>
                <Select value={customer} onValueChange={(v) => setValue("customer", v, { shouldValidate: true })}>
                  <SelectTrigger className={cn("h-8 text-xs border-border", errors.customer && "border-destructive ring-1 ring-destructive")}>
                ...
                <FormError message={errors.customer?.message} />
              </div>
              <div className="space-y-1 md:col-span-2">
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground">配送先住所</label>
                <Input {...register("deliveryAddr")} placeholder="配送先住所を入力してください" className={cn("h-8 text-xs border-border", errors.deliveryAddr && "border-destructive")} />
                <FormError message={errors.deliveryAddr?.message} />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground">出発地倉庫 <span className="text-destructive">*</span></label>
                <Select value={warehouse} onValueChange={(v) => setValue("warehouse", v, { shouldValidate: true })}>
                  <SelectTrigger className={cn("h-8 text-xs border-border", errors.warehouse && "border-destructive ring-1 ring-destructive")}>
              ...
              <div className="col-span-2 md:col-span-4 space-y-1">
                ...
                <Textarea {...register("remark")} placeholder="出庫伝票に関する備考事項を入力してください" className={cn("text-xs border-border min-h-[60px] resize-none", errors.remark && "border-destructive")} />
        ...
        <ItemSelectModal open={isItemModalOpen} onOpenChange={setIsItemModalOpen} items={PRODUCT_CATALOG} onSelect={addItem} selectedCodes={details.map((d) => d.itemCode)} showStock={true} title="出庫品目選択" />
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {details.map((item) => {
                        ...
                        return (
                          <TableRow key={item.id} className={cn("border-border hover:bg-secondary/50", overStock && "bg-destructive/5")}>
                            ...
                            <TableCell className="px-3 py-2 text-xs text-right font-mono">
                              <span className={item.stockQty <= 5 ? "text-warning" : "text-success"}>{item.stockQty.toLocaleString()}</span>
                            </TableCell>
                            <TableCell className="px-3 py-2 text-xs text-right">
                              <div className="flex items-center justify-end gap-1">
                                {overStock && <AlertTriangle className="w-3 h-3 text-destructive" />}
                                <Input type="number" min={1} value={item.shipQty} onChange={(e) => updateShipQty(item.id, Math.max(1, parseInt(e.target.value) || 1))} className={cn("h-6 w-20 text-xs text-right border-border ml-auto", overStock && "border-destructive text-destructive")} />
                              </div>
                            </TableCell>
                            ...
                            <TableCell className="px-3 py-2 text-center">
                              <Checkbox checked={item.inTransit} onCheckedChange={(checked) => updateTransit(item.id, !!checked)} className="border-border" />
                            </TableCell>
                            <TableCell className="px-3 py-2 text-xs">
                              <Input value={item.lotNo} onChange={(e) => updateLot(item.id, e.target.value)} placeholder="LOT-" className="h-6 w-32 text-xs font-mono border-border" />
                            </TableCell>
        ...
        <div className="flex items-center justify-end gap-2 pb-4">
          <Button variant="outline" size="sm" onClick={() => navigate("/production/shipping")} className="gap-1.5 text-xs">キャンセル</Button>
          ...
          <Button
            ...
            disabled={!isValid || details.length === 0 || hasStockWarning}
